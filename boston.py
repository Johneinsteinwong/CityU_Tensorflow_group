...
Categorical input and numeric input are segregated (separation
approach)
"""
...
with graph.as_default():
    ...
    tf_train_cate_data = tf.placeholder(tf.float32, shape=(batch_size,cate_size))
    
    l1_weights = tf.Variable(tf.random_uniform(shape=[feature_size, num_hidden],minval=-0.1,maxval=0.1))
    l1_bias = tf.Variable(tf.zeros(shape=[num_hidden]))
      
    l2_weights = tf.Variable(tf.random_uniform(shape=[num_hidden,cate_size],minval=-0.1,maxval=0.1))
    l2_bias = tf.Variable(tf.zeros(shape=[cate_size]))
    
    tf_valid_dataset = tf.constant(valid_dataset)
    tf_test_dataset = tf.constant(test_dataset)

    tf_valid_cate_data = tf.constant(valid_dum)
    tf_test_cate_data = tf.constant(test_dum)
    
    def model1(data):
        hidden1 = tf.matmul(data,l1_weights) + l1_bias
        hidden2 = tf.matmul(hidden1,l2_weights) + l2_bias
        return tf.diag_part(tf.matmul(hidden2, tf.transpose(tf_train_cate_data)))
    
    def model_valid(data):
        hidden1 = tf.matmul(data,l1_weights) + l1_bias
        hidden2 = tf.matmul(hidden1,l2_weights) + l2_bias
        return tf.diag_part(tf.matmul(hidden2, tf.transpose(tf_valid_cate_data))) 
    
    def model_test(data):
        hidden1 = tf.matmul(data,l1_weights) + l1_bias
        hidden2 = tf.matmul(hidden1,l2_weights) + l2_bias
        return tf.diag_part(tf.matmul(hidden2, tf.transpose(tf_test_cate_data))) 
    ...
    valid_prediction = model_valid(tf_valid_dataset)
    test_prediction = model_test(tf_test_dataset)
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps):
       ...
       feed_dict = {tf_train_dataset : train_dataset, tf_train_labels : train_labels, tf_train_cate_data : train_dum}
       ...
       if (step % 1 == 0):
            ...
            print('Minibatch RMSE: %.4f' % rmse(predictions, train_labels))
